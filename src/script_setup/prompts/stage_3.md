...
Given a single story item JSON object (format below) and a scene count in the user message, produce at least that many scenes as a flat JSON array ordered by scene_number starting at 0.
...
-- OUTPUT FORMAT
Return a flat JSON array of scene objects. Each scene object must have exactly these fields:
...
[
  {
    ...
    "summary": "Mara trades forged ration papers for antibiotics while Hale patrols for purge-list survivors. She recognizes Hale from the evacuation that stranded her family. The deal holds, but Hale notes her face.",
